...
const Pago = () => {
  ...
  const procesarPago = () => {
    if (!nombre || !cantidad || !imagen || !metodoSeleccionado) {
      alert(
        "Por favor completa todos los campos, selecciona un método y adjunta el comprobante."
      );
      return;
    }
  ...
  return (
    <div className="relative w-full min-h-screen py-12 px-4 md:px-0 bg-gradient-to-b from-[#F0FAFF] via-[#F8FDFF] to-[#F0FAFF] flex justify-center">
      <div className="relative w-full max-w-4xl p-8 md:p-12 rounded-3xl shadow-2xl bg-white overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {pagos.map((pago) => (
              <div
                ...
                className={`relative cursor-pointer flex items-center gap-4 p-6 rounded-2xl transition-all duration-300 ${
